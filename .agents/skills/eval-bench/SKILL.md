...
**Empty means empty (decided 2026-09-09).** A gold key of `[]` is a real key, not a missing one: the answer rests on something being absent from the transcript (NA because the trigger never arose, fail by omission, pass because the behaviour never occurred) and the expected evidence is the empty list. Score such pairs as evidence-correct only when the model also emits `[]`; any cited line is a false positive. Do not exclude empty-key pairs from the evidence metric and do not fill them with near-miss lines. Report the empty-key share of the benchmark alongside the metric so the two populations stay visible (benchmark gold at freeze: 2,907 of 20,700 pairs empty). Known limitation: labellers were not instructed on this, so some absence answers in the gold do cite near-miss lines; the gold is what the labeller wrote.
