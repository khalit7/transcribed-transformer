...
The evaluation harness's data lives in `data/benchmark/`, generated by `src/synthesis/` (`uv run python -m src.synthesis.synth_data <model_name> <generation_size>`). Nothing under the directory is tracked in git; a releasable Track P copy is produced by `python -m src.synthesis.export --track p`.

- **`labelled_data.jsonl`** — the benchmark: one self-contained record per label, `id = <call>::<question_id>`:
  - `source_id` — where the call came from (corpus / config / locale / document id); each transcript variant's `origin` says which layer supplied its lines;
  - `track` (`track-p` / `track-nc`);
  - `question` — the full question: `id`, `source` (`bank` = reusable, applies to any call; `written` = written by the generator for this call), `family` (`vulnerability` / `complaint_and_eod` / `general_qa`), `text`, `description`, `options` with grading criteria;
  - `transcript` — the line-aligned `variants` (`clean`, `messy`, each with its `origin`), the hidden `speaker_roles`, and the `tag_policy` used to assign `SPEAKER_NN` tags to roles;
  - `label` — `answer`, `evidence` (1-based line numbers), `summary`. **Evidence keys are never guaranteed complete**, injected labels included: the inserted lines certainly support the answer, but the original call may contain further supporting lines that were not checked for. Score evidence as precision against the key; recall is not measurable on this benchmark;
  - `generation_info` — `name` (model), `mode`, `cost_usd` for this record's call, `timestamp`. `mode` is the label's provenance: `injected` means turns were written into the call so the answer holds (by construction; `evidence` is the inserted lines), `as_is` means the call was judged as it stood (LLM judgement). Which lines of a call are synthetic is read from that call's `injected` record (same `<call>::` prefix).
- **`questions.jsonl`** — derived from the above: the bank (source of truth: `src/synthesis/question_bank.py`) plus every distinct question written for a call. Regenerated after each generation run.

**Task shape.** A case is one call (v0; multi-call cases are v1) rendered one turn per line as `SPEAKER_NN: text`. Which tag the agent gets is decided per case by the tag policy (`random` by default, so roles must be inferred as with real diarisation; `agent_first` is available). Roles are hidden metadata. Every case has one or more **line-aligned variants**: `clean` (verbatim/human) and `messy` (ASR-style); line *i* is the same turn in every variant, so one evidence key serves all of them and the clean-vs-messy accuracy gap is measurable per system. A question carries its text, an optional description, and its options with grading criteria; **answers are always drawn from `pass | fail | partial_pass | NA`**, every question offers at least pass and fail, the other two only where meaningful. An assessment is `{answer, evidence (1-based line numbers, ascending, empty when nothing supports), summary, provenance}`.

**Question bank (v0, 3 questions)**: one per family, kept deliberately small while the generator is being shaped — `vul-health-01` (health-condition disclosure; vulnerability), `cmp-raised-01` (complaint made; complaint_and_eod), `gen-dob-01` (date of birth established; general_qa). Original text in the generic shape of regulated-industry call QA; none copied from any internal source. A question is just a question: `id`, `source` (`bank` | `written`), `family`, `text`, `description`, `options` with criteria. The generator works from the criteria alone; there are no separate injection recipes.

**Label provenance is `generation_info.mode` × `question.source`**, four cells, all produced by one prompt template (`src/synthesis/generate.py`) with two optional blocks — *write the question* and *write turns*:
- `injected` × `bank` — a reusable question's target answer is made true by writing 1–4 turns into the call; the generator picks a target whose rule can be made true by adding turns (never an absence), with a preferred target supplied for balance. Label by construction; no injection recipes exist beyond the question's own grading criteria.
- `injected` × `written` — the generator invents a question that fits the specific conversation *and* the turns that make its target answer hold. By construction, single-use question.
- `as_is` × `bank` — a reusable question judged on the call as it stands. LLM judgement.
- `as_is` × `written` — a question written about the call and judged. LLM judgement.
- `gold` (human) — none yet; would be recorded as `generation_info.name = "human"`.

**Case sources** (any conversation between different people qualifies): AppTek (P+SA; clean = verbatim, **messy = real Whisper output time-aligned to the verbatim turns**, so the messy twin is genuine ASR); Taskmaster (P, tier 3; messy = channel v2.2 applied per line, synthetic); ACI-Bench (P; messy = channel, synthetic); SPoRC (NC; real diarised ASR, single messy variant). Injected turns in messy variants are channel-noised with crude surface restoration, which is a known weak point (the channel failed its gate) and is visibly more garbled than the surrounding real-ASR lines in AppTek cases; TTS→ASR is the planned upgrade. SPoRC podcasts are poor material for *service-conduct* questions (the judge correctly returns NA/"not a customer call"); vulnerability injections work there.

**State (2026-09-02)**: generator built and tested end to end — a 4-case run over all four sources produced 12 assessments (4 by construction, 8 silver) for $1.01 in 134 s, i.e. about $0.25 per case at three generator calls each (each `claude -p` call carries the CLI's ~33k-token cached system prompt). `labelled_data.jsonl` was cleared on 2026-09-02 after the simplification pass; the benchmark is unpopulated until a sized run is requested. Reserved for evaluation: AppTek cases are never training text.
