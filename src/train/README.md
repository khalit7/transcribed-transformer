...
```
torchrun --nproc_per_node 2 -m src.train.train configs/e1/qwen3-1.7b-base.yaml              # train
...
| `hybrid.py` | E6's decoder-only + encoder model: a Gemma 3 decoder whose layers are wrapped with a cross-attention sublayer over a T5Gemma 2 text encoder, gated on the post-norm weight (closed at start, so step 0 is the E1 model exactly); FlashAttention varlen for the cross-attention in training, save/load |
| `stitched.py`, `fit_stitch.py` | E4-mix-and-match: a T5Gemma 2 encoder of one size with the T5Gemma 2 decoder of another, joined by an affine stitch on the encoder output; `fit_stitch.py` fits the stitch by ridge regression onto the decoder-side encoder's states before training and reports the held-out explained variance |
| `generate.py` | Greedy generation for a split file (vLLM; `--backend prefixlm` for E2; `--backend encdec` for E3; `--backend hf_encdec` for a native Hugging Face encoder-decoder such as T5Gemma 2 (E4) and `hf_encdec_hybrid` for the E6 native hybrid; `--backend hybrid` and `--backend stitched` for the models above; HF as the slow fallback) |
| `evaluate.py` | Scoring and slicing; `metrics.json` and `results.md` |
| `../../configs/e1/*.yaml`, `e2/`, `e3/`, `e4/`, `e6/`, `adapt/` | E1, E2 and E3 runs (Qwen3-1.7B-Base on every training record; E2 differs by `prefix_lm: true`, E3 by `arch: encdec`), their adapted variants, and the adaptation stages (causal, MNTP, seq2seq). `e4/t5gemma2-*.yaml` (`arch: hf_encdec`, Google's converged encoder-decoders through the library's own forward, sdpa, DDP) with their controls `e1/gemma3-*.yaml` (the decoders they were adapted from) form the native pairs; `e4/mm-*.yaml` (`arch: stitched`) are the mix-and-match arms; `e6/*.yaml` are the hybrids (`arch: hf_encdec` with `decoder_sees_prompt: true`, or `arch: hybrid`). None of the Gemma-family arms is comparable with the Qwen arms |
