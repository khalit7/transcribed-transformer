...
| `generate.py` | Greedy generation for a split file (vLLM; `--backend prefixlm` for E2, bidirectional prefill then cached decoding; HF as the slow fallback) |
| `evaluate.py` | Scoring and slicing; `metrics.json` and `results.md` |
| `../../configs/e1/*.yaml`, `e2/`, `adapt/` | E1 and E2 runs (Qwen3-1.7B-Base on every training record; E2 differs by `prefix_lm: true`), their adapted variants, and the two adaptation stages |
