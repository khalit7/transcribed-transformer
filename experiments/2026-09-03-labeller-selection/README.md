...
- Judge: `claude:opus`, grading each labeller's answer, evidence, summary and (vulnerability) tags on a stratified sample of pairs, all labellers' outputs for a pair in one call; sample size set once labels exist. Then my own human review of a subset of the judged pairs, to calibrate the judge before its verdicts are trusted.
...
**Human review** (2026-09-04): I read the judgement file (`opus_400.jsonl`, judge answers, grades and reasons per label) directly rather than grading the exported 40 pairs blind, and agreed with the judge in most cases; the judge's verdicts are therefore taken as the reference. This is an informal calibration: there is no per-criterion agreement figure, and the blind export was not used.
...
- **Decision taken (2026-09-04, D22 in notes)**: qwen3.8 labels general_qa; vulnerability, complaint and eod are labelled exclusively by Sonnet; `synth_data` takes the families as a required argument and both runs cover the same transcripts.
