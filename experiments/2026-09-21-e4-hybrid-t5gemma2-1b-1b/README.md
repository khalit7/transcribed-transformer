# E4: hybrid encoder-decoder, T5Gemma 2 1b-1b with the decoder reading the prompt too

Written before the run, 2026-09-21. The idea: an encoder-decoder in which the decoder also gets the full input. The encoder encodes the input; the decoder sees the raw input through self-attention and the encoder's states through cross-attention. (Called E3h while it was being planned, renamed E4.)
...
- If the encoder's states add information: E4 at or above the 1b-1b on evidence F1 on both variants and on clean unseen-question macro-F1 (1b-1b: 0.619 / 0.590; 0.677).
- If the advantage is division of labour: E4 falls back toward the Gemma 3 1B decoder (0.530 / 0.500 evidence F1; 0.496 clean unseen), because the decoder now carries the prompt again and can shortcut past the encoder.
- **The hypothesis "the encoder adds information" is disconfirmed if** E4 is below the 1b-1b on evidence F1 on both variants by more than 0.01. A result between the two parents is reported as partial.

## Setup

- Weights: `google/t5gemma-2-1b-1b`, the same starting point as the native E3 run; no new parameters. The decoder's merged attention (one softmax over its own keys and the encoder's) already covers both paths.
- Decoder input: start token, the prompt, then the label; loss on the label only (`model.decoder_sees_prompt: true`, `collate_encdec(dec_prompt=True)`). Causal self-attention over prompt and label, as the decoder was pretrained; the encoder reads the prompt as before. Everything else as `configs/e3/t5gemma2-1b-1b.yaml`: E1's recipe, 16k cap, DDP, SDPA.
