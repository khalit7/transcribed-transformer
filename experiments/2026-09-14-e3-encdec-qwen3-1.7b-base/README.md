...
The encoder-decoder arm of the README's "Design" table, written before the run on 2026-09-14. Khalid's reading of the prefix-LM line (E2, E2+, E2++): the bidirectional arm improved monotonically with adaptation and reached parity on answers with a small lead on evidence, so bidirectional reading has "a little bit of promise" and deserves a dedicated encoder.
...
- Adaptation: [tt-pretrain/1gyti0ha](https://wandb.ai/khalit7-/tt-pretrain/runs/1gyti0ha), 2026-09-15 00:02 → 19:50 (19.8 h; a first full-parameter attempt of 20:30–23:55 on the 14th was discarded at Khalid's request so that the two halves of the objective would be logged separately; the LoRA seq2seq-only attempt of 15:17–19:45 is kept under `checkpoints/adapt-encdec-seq2seq-aborted`). 4,107 steps, 661.5M tokens, all 3.79B parameters, FSDP-sharded, 9.5k tokens/s, peak 29.8 GiB per GPU. Held-out val, seq2seq / masked / combined (token-weighted): step 0 2.160 / 10.015 / 5.613; 250 1.905 / 1.631 / 1.784; 500 1.871 / 1.437 / 1.680; 1000 1.832 / 1.306 / 1.601; 2000 1.798 / 1.219 / 1.543; 3000 1.779 / 1.182 / 1.517; **end 1.770 / 1.168 / 1.505**. The masked half ends where E2++'s masked-only stage ended (1.165), so the encoder is as repaired as E2++'s; the seq2seq half passed the abandoned seq2seq-only run's step-1000 value (1.895) by step 250 and ended 0.12 below it; both halves flat over the last 1,000 steps.
...
- E4 and E5 stay behind their probe gate; nothing here argues for opening it.
